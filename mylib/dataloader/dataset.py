...
import numpy as np
from mylib.dataloader.path_manager import PATH, TESTPATH
...
class ClfDataset(Sequence):
    def __init__(self, crop_size=32, move=3, subset=[0],
...
def get_test_loader(dataset, batch_size):
    ...
    def take(n):
        for i in range(n):
            yield i
    index_generator = take(total_size)
    while True:
        data = []
        for _ in range(batch_size):
            idx = next(index_generator)
